...
class StatsView(APIView):
    ...
    def get_global_stats(self, date_obj=None):
        ...
        guests = Guest.objects.filter(stays__status='active').distinct()
    ...
    def get_hotel_stats(self, hotel, date_obj=None):
        guests = Guest.objects.filter(stays__hotel=hotel, stays__status='active').distinct()
    ...
    def get_guest_stats(self, hotel, date_obj=None):
        guests = Guest.objects.filter(stays__hotel=hotel, stays__status='active')
        if date_obj:
            guests = guests.filter(first_contact_date__date=date_obj)
